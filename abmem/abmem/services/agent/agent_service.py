...
def calculateOffers(agent: Agent, prediction: int) -> [Offer]:
    ...
    for plant in agent.portfolio.plant_set.all():
        lowerBound = int(plant.resource.fuelCost)
        offer = OfferFactory.create(agent=agent, resource=plant.resource, amount=plant.capacity, offerPrice=decimal.Decimal(random.randint(lowerBound,250)))
        offers.append(offer)
    # Offer Module
    return offers
